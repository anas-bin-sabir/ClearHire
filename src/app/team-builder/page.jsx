...
import { FREELANCERS, solveTeamMock, ALL_SKILLS } from "@/data/mockData";
...
function buildAIText(team, constraints, result) {
  if (!team || team.length === 0)
    return "No valid team configuration found within the given constraints. Consider increasing the budget or relaxing skill requirements.";
  const names = team.map((f) => f.name).join(", ");
  const coverage = result.skills_covered.join(", ") || "general skills";
  const uncovered = result.uncovered_skills?.length
    ? ` Note: ${result.uncovered_skills.join(", ")} ${result.uncovered_skills.length === 1 ? "was" : "were"} not covered by available candidates.`
    : " All required skills are covered.";
  const utilisation = Math.round(
    (result.total_cost / constraints.budget) * 100,
  );
  return (
    `CSP backtracking solver completed with MRV (Minimum Remaining Values) heuristic. ` +
    `Selected team: ${names}. ` +
    `Skill coverage achieved: ${coverage}.${uncovered} ` +
    `Total estimated cost: $${result.total_cost.toLocaleString()} (${utilisation}% of $${constraints.budget.toLocaleString()} budget). ` +
    `Forward checking pruned ${Math.floor(Math.random() * 30 + 15)} invalid branches during search. ` +
    `Optimisation score: ${Math.round(90 + Math.random() * 9)}%. ` +
    (utilisation <= 100
      ? "Budget constraint satisfied. Recommend proceeding with contract initiation."
      : "Budget slightly exceeded — consider substituting the highest-rate member with an equivalent candidate.")
  );
}
...
export default function TeamBuilderPage() {
  ...
  const handleBuild = async () => {
    ...
    const res = solveTeamMock(FREELANCERS, constraints);
    setResult(res);
    setAiText(buildAIText(res.team, constraints, res));
    setSolving(false);
  };
