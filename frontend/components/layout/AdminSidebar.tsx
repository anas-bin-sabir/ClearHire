      ...
      <div className={`pt-2 pb-5 border-b border-white/5 flex items-center gap-3 ${isMinimized ? 'justify-center px-4' : 'px-6'}`}>
        ...
        {!isMinimized && (
          <div className="min-w-0">
            ...
            <span className="text-[10px] text-rose font-mono tracking-widest font-semibold uppercase block truncate">Admin Console</span>
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 px-3 py-6 space-y-1.5 overflow-y-auto">
        {navItems.map(item => {
          ...
          return (
            ...
            >
              <Icon className={`h-4.5 w-4.5 transition-colors shrink-0 ${Active ? 'text-rose' : 'text-slate-400 group-hover:text-white'}`} />
      ...
      {isMinimized ? (
        <div className="p-4 border-t border-white/5 bg-ink/40 flex flex-col items-center gap-4">
          <div className="relative group">
            ...
            <span className="sidebar-tooltip group-hover:scale-100">
              {session?.user?.name || 'Admin'} ({session?.user?.email || 'admin@demo.com'})
            </span>
          </div>
